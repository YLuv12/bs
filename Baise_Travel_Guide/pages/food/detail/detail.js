...
Page({
  ...
  onLoad(options) {
    this.setData({ id: options.id, lang: app.globalData.lang || 'zh' })
    this.loadDetail()
  },
  ...
  async loadDetail() {
    ...
    try {
      const { id } = this.data
      const detailData = await get(`/food/detail/${id}`)
  ...
  toggleGrass() {
    ...
    if (this.data.isGrassed) return
    this.setData({ isGrassed: true })
    wx.showToast({ title: this.data.lang === 'zh' ? '种草成功' : 'Marked successfully', icon: 'success' })
  },
  ...
  goToComment() {
    ...
    wx.navigateTo({ url: `/pages/comment/edit/edit?type=food&id=${this.data.id}` })
  },
  ...
  onShareAppMessage() {
    ...
    return {
      ...
      path: `/pages/food/detail/detail?id=${detail.id}`,
